//---------------------------------------------------------------------
// <copyright file="ProcessEx.cs" company="Microsoft">
...
/// <summary>
/// A wrapper around <see cref="System.Diagnostics.Process"/> that captures stdout and stderr and provides a way to wait for the process to exit.
/// </summary>
internal sealed class ProcessEx : IDisposable
{
    ...
    {
        var packageRestorePath = typeof(ProcessEx).Assembly
            .GetCustomAttributes<AssemblyMetadataAttribute>()
            .FirstOrDefault(attribute => attribute.Key == "TestPackageRestorePath")
            ?.Value;

        Assert.NotNull(packageRestorePath);

        return packageRestorePath;
    }
